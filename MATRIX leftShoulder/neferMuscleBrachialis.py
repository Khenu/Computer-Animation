# neferMuscleBrachialis.py
# Created by Laushon Neferkara on 9/28/13.
...
def main():
	...
	muscle = NeferMuscle('L_brachialisA', 5, n3driver)
...
musNameFrom = 'L_brachialis'
musNameTo = 'L_brachialisA'
numCtrls = 5

for ctrlNum in range(numCtrls):
	for axis1Point in ('x0', 'x45', 'x90', 'x135', 'x180', 'xn45'):
		for axis2Point in ('y0', 'y45', 'y90', 'y135', 'y170'):
			for axis3Point in ('w0', 'w45', 'w90', 'wn45', 'wn90'):
				for trans in ['translateX', 'translateY', 'translateZ']:
					transValue = mc.getAttr('%s_control%s_%s_%s_%s_target.%s' % (musNameFrom, str(ctrlNum + 1), 
						axis1Point, axis2Point, axis3Point, trans))
					mc.setAttr('%s_control%s_%s_%s_%s_target.%s' % (musNameTo, str(ctrlNum + 1), 
						axis1Point, axis2Point, axis3Point, trans), transValue)
