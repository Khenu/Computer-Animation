# neferMuscleTricepsMedialNew2.py
# Created by Laushon Neferkara on 9/26/13.
...
class MuscleControl():
	...
	def __init__(self, muscleName, cIndex, nDriver):	
		...
		self.name = 'iControlMidMus_%s%s1' % (muscleName, str(cIndex))
		self.autoGrpName = 'grpiControlMidMus_%s%sAUTO1' % (muscleName, str(cIndex))
...
class NeferMuscle():
	'''Represents a neferMuscle: a Maya Muscle that is controlled by a multi-variable
	 pose controller.'''
	def __init__(self, muscleName, controlIndexList, driverInfo):
		self.muscleName = muscleName
		self.controlIndexList = controlIndexList
	...
	def getMayaMusInfo(self):
		...
		self.musCtrls = []
		self.musCross = []
		for cIndex in self.controlIndexList:
	...
	def createPoses(self):
		...
		def _createCtrlCrossPoses(parentGrpName, driverPt):
			...
			for dIndex in range(len(self.controlIndexList)):
				ctrlPoseGrp = SimpleGrp(
					parentGrpName.replace('_grp', '_control%s_grp' % str(self.controlIndexList[dIndex])), 
					parentGrpName, 
					False)
				...
				ctrlTargetName = '%s_control%s_%s_target' % (self.muscleName, str(self.controlIndexList[dIndex]), driverPt)
				ctrlTarget = MuscleCtrlTarget(ctrlTargetName, self.musCtrls[dIndex], ctrlPoseGrp.name)
				
				# Create cross section target
				crossTargetName = '%s_crossSection%s_%s_target' % (self.muscleName, str(self.controlIndexList[dIndex]), 
					driverPt)
	...
	def connectDriver(self):
		...
		for index in range(len(self.controlIndexList)):
...
def main():
	...
	muscle = NeferMuscle('L_tricepsBrachiiMedial2', [1, 2, 3, 4, 5], n3driver)
