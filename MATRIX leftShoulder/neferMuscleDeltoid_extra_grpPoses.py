# neferMuscleDeltoid_extra_grpPoses.py
# Created by Laushon Neferkara on 10/10/13.
# Copyright (c) 2013 Skin+Bones Modeling and Rigging Company. All rights reserved.

'''
Controls the _extra group above the muscle controls. Used to correct the changing
orientation of the controls of the deltoid posterior.
Uses N3_muscleDriver6.

I could add translation to the _extra groups to improve the deformation.
...
class MuscleControl():
	...
	def __init__(self, muscleName, cIndex, nDriver):	
		...
		self.name = 'iControlMidMus_%s%s1_extra_grp' % (muscleName, str(cIndex + 1))
	...
	def connectTargets(self):
		...
		# mc.parentConstraint(self.targetList, self.name, weight=0.0)
		mc.pointConstraint(self.targetList, self.name, weight=0.0)
	...
	def connectDriver(self):
		...
		for pIndex in range(len(self.targetList)):
			...
			# mc.connectAttr(
			# 	'%s.%s' % (self.nDriver, self.driverList[pIndex]),
			# 	'%s_parentConstraint1.%sW%s' % (self.name, self.targetList[pIndex], str(pIndex))
			# 	)
			##########################################################################################
			mc.connectAttr(
				'%s.%s' % (self.nDriver, self.driverList[pIndex]),
...
class MuscleCtrlTarget(SceneTarget):
	...
	def __init__(self, name, musCtrl, parentGrpName):	
		# self.extraTarget = 'iControlMidMus_%s'
	...
	def createPoses(self):
		...
		def _createCtrlCrossPoses(parentGrpName, driverPt):
			...
			for dIndex in range(self.numCtrls):
				...
				ctrlPoseGrpTarget = 'grpiControlMidMus_%s%sAUTO1' % (self.muscleName, str(dIndex + 1))
				ctrlPoseGrp.parentConstraint(ctrlPoseGrpTarget)
				
				# Create control target
				ctrlTargetName = '%s_extra_control%s_%s_target' % (self.muscleName, str(dIndex + 1), driverPt)
		...
		mainPoseGrp = SimpleGrp('%s_extra_pose_grp' % self.muscleName, topPoseGrp)
